...
public class Main {
   ...
   static class Map{
      ...
      private List<Node> moveNumber(List<Node> square, int dir){
         if(dir == DIR_CLOCK){
            int preNum = square.get(square.size()-1).num; // 넣을 숫자
            for(int to = square.size()-2; to >=0; to--){ // 리스트의 뒤에서부터 앞으로 이동한다.
            ...
            int preNum = square.get(0).num; //넣을 숫자
            for(int to = 1; to < square.size(); to++){ // 리스트의 앞에서 뒤로 이동
               int tmp = square.get(to).num;
               square.get(to).num = preNum;
               preNum = tmp;
            }
